 ...
 * @author jitor
 */
public class Gamelvl6 implements Initializable {
   ...
   private void initializeLabels() {
    ...
    private void saveStartLevel(){
         ...
         for(int row = 0; row < 11; row++){
             for(int column = 0; column < 19; column ++){
                 startLevel[row][column] = level6[row][column];   
            }      
        }    
    }
       ...
       private void savePreviousMove(){
        for (int row = 0; row < 11; row++) {
            for (int column = 0; column < 19; column++) {
                previousMatrix[row][column] = level6[row][column];
            }
        }
    }
    ...
    private void searchPlayer(){
        ...
        for (int row = 0; row < 11; row++) {
            for (int column = 0; column < 19; column++) {
                System.out.print(level6[row][column] + ",");
                if (level6[row][column] == 'P' || level6[row][column] == 'F') {    
                    playerRow = row;
                    playerColumn = column;
                    break;
                }    
   ...
   @FXML
    ...
    public void  finishedLevel(){
        ...
        if((level6[1][4] == 'M') && (level6[9][5] == 'M')&&
            (level6[2][10] == 'M') && (level6[4][11] == 'M')){
             btnSave.setDisable(true);
              btnRestart.setDisable(true);
    ...
    private void playerLeavingTheGoal(int newRow, int newColumn){
        ...
        if(level6[newRow][newColumn] == ' ' && level6[playerRow][playerColumn] == 'F'){
   
            savePreviousMove();
            level6[newRow][newColumn] = 'P';
            level6[playerRow][playerColumn] = '+';     
        }
    }
    
    public void playerMove(int newRow, int newColumn){
        
        if(level6[newRow][newColumn] == ' '){ 
    ...
    public void playerEnteringTheGoal(int newRow, int newColumn){
        ...
        if((level6[newRow][newColumn] == '+') && (level6[playerRow][playerColumn] == 'F')){
            ...
            savePreviousMove();
            btnOneStepBack.setDisable(false);
            level6[newRow][newColumn] = 'F';
            level6[playerRow][playerColumn] = '+'; 
        }
        else if(level6[newRow][newColumn] == '+'){
    ...
    public void checkIfBoxIsPushed(int newRow, int newColumn,int aux1,int aux2){
        ...
        if((level6[newRow][newColumn] == 'O' || level6[newRow][newColumn] == 'M') && level6[newRow + aux1][newColumn + aux2] != '#' 
            && level6[newRow + aux1][newColumn + aux2] != 'O' && level6[newRow + aux1][newColumn + aux2] != 'M'){
    ...
    private void pushOutGoalBox(int newRow, int newColumn, int aux1, int aux2){
        ...
        if(level6[newRow + aux1][newColumn + aux2] == '+' && level6[newRow][newColumn] == 'M' &&  level6[playerRow][playerColumn] == 'F'){
                
            savePreviousMove();
            level6[newRow + aux1][newColumn + aux2] = 'M';
            level6[newRow][newColumn] = 'F';
            level6[playerRow][playerColumn] = '+';
        }else if(level6[newRow + aux1][newColumn + aux2] == ' ' && level6[newRow][newColumn] == 'M' &&  level6[playerRow][playerColumn] == 'F'){  
            
            savePreviousMove();
            level6[newRow + aux1][newColumn + aux2] = 'O';
            level6[newRow][newColumn] = 'F';
            level6[playerRow][playerColumn] = '+';          
        }else if(level6[newRow + aux1][newColumn + aux2] == '+' && level6[newRow][newColumn] == 'M'){
            
            savePreviousMove();
            level6[newRow + aux1][newColumn + aux2] = 'M';
            level6[newRow][newColumn] = 'F';
            level6[playerRow][playerColumn] = ' '; 
        }else if(level6[newRow][newColumn] == 'M'){   
    ...
    private void pushBox(int newRow, int newColumn, int aux1, int aux2){
        ...
        if(level6[newRow + aux1][newColumn + aux2] == ' ' && level6[playerRow][playerColumn] == 'F'){
                
            savePreviousMove();
            level6[newRow + aux1][newColumn + aux2] = 'O';
            level6[newRow][newColumn] = 'P';
            level6[playerRow][playerColumn] = '+';
        }else if(level6[newRow + aux1][newColumn + aux2] == ' '){
    ...
    private void pushBoxToTheGoal(int newRow, int newColumn, int aux1, int aux2){
        ...
        if(level6[newRow + aux1][newColumn + aux2] == '+'){
    ...
    public void counterSteps(int newRow, int newColumn){
         ...
         if(level6[newRow][newColumn] != '#' &&  level6[newRow][newColumn] != 'O' &&  level6[newRow][newColumn] != 'M'){
            steps--;
            labSteps.setText(String.valueOf(steps));
        }
    }
    ...
    private void restartLevel(){
        ...
        for (int row = 0; row < 11; row++) {
            for (int column = 0; column < 19; column++) {
                level6[row][column] = startLevel[row][column];
            }
        }
        updateMap();  
    }
    ...
    private void gameLocked(){
        ...
        if(level6[3][4] == 'O' || level6[2][8] == 'O' || level6[3][8] == 'O' || level6[9][8] == 'O' || 
        level6[8][7] == 'O' || level6[7][9] == 'O' || level6[2][8] == 'O' || level6[5][12] == 'O' || level6[6][12] == 'O' ){
   ...
   private void saveGame(){
        ...
        String matrixlevel1 = matrixToString(level6);
    ...
    public void updateGame(){
        ...
        String matrixLevel5 = matrixToString(level6);
        
        String playerGame = matrixLevel5;
    ...
    public void loadGame(){
        ...
        if(option == 7){
            labPlayerName.setText(name);
            level6 = stringToMatrix(playerMatrix,11,19);
    ...
    private void clicklOneStepBack(ActionEvent event) {
        ...
        for (int row = 0; row < 11; row++) {
            for (int column = 0; column < 19; column++) {
                level6[row][column] = previousMatrix[row][column];
            }
        }
        updateMap();
